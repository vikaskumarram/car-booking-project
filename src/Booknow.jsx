...
// App.jsx se bheja gaya isLoggedIn yahan receive karenge
export function Booknow({ isLoggedIn }) { 
  ...
  const handleChoose = (car) => {
    // 🛡️ Login Check Logic
    if (isLoggedIn) {
      // Agar login hai toh car data ke saath home page par bhejo
      navigate("/", { state: { selectedCar: car } });
    } else {
      // Agar login nahi hai toh ye message dikhao
      alert("⚠️ First login your account or Signup to book a ride!");
      navigate("/login"); // User ko login page par redirect kar do
    }
  };
